...
//            return new TextMessage("kamu abis ketawa ngakak yaa, " + senderName + "?");
            ...
            SortedSet<Map.Entry<String,Integer>> groomUser = entriesSortedByValues(laughers.get(groomUserId));
...
//        return new TextMessage(event.getMessage().getText() + " : " + senderName);
    ...
    public String getSenderName(MessageEvent<TextMessageContent> event) throws ExecutionException, InterruptedException{
        UserProfileResponse senderProfile;
        if(event.getSource() instanceof GroupSource){
            String groupId = getGroomUserId(event);
            String senderId = event.getSource().getUserId();
            CompletableFuture<UserProfileResponse> senderProfileFuture = lineMessagingClient.getGroupMemberProfile(groupId, senderId);
            senderProfile = senderProfileFuture.get();
        }
        else if(event.getSource() instanceof RoomSource){
            String roomId = getGroomUserId(event);
            String senderId = event.getSource().getUserId();
            CompletableFuture<UserProfileResponse> senderProfileFuture = lineMessagingClient.getRoomMemberProfile(roomId, senderId);
            senderProfile = senderProfileFuture.get();
        }
        else{
            String userId = getGroomUserId(event);
            CompletableFuture<UserProfileResponse> senderProfileFuture = lineMessagingClient.getProfile(userId);
            senderProfile = senderProfileFuture.get();
        }
    ...
    public void putLaughter(String senderName, String groomUserId){
        ...
        //if someone had already laughed in the group before
        else{
            ...
            //if he had laughed before
                ...
                laughers.get(groomUserId).put(senderName, laughers.get(groomUserId).get(senderName)+1);
            }
