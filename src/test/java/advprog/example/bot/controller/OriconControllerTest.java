...
public class OriconControllerTest extends TestCase {
    ...
    public void testHandleTextMessageEvent() {
        ...
        MessageEvent<TextMessageContent> event = EventTestUtil
                .createDummyTextMessage("/oricon comic 2018-05-14");
        ...
        assertEquals("Ranking does not available at this date" 
                + ", Please Input a date on Monday", reply.getText());

        event = EventTestUtil.createDummyTextMessage("wrong input");
        reply = oriconController.handleTextMessageEvent(event);
        assertEquals("Wrong Input,Please Input with correct format " 
                + "'/oricon comic YYYY/MM/DD'", reply.getText());
    }
